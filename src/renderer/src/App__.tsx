...
// =============================================================================
//  CODE GENERATOR ENGINE (Now with C# and Houdini)
// =============================================================================

const generateCodeBlocks = (nodes: Node<FPNodeData>[], edges: Edge[], mode: 'js_toonboom' | 'py_maya' | 'py_houdini' | 'cs_csharp'): CodeBlock[] => {
  const blocks: CodeBlock[] = [];
  const processedIds = new Set<string>(); 

  // Set the correct header based on the target software
  let headerText = "";
  if (mode === 'js_toonboom') {
...
export default function App() {
  ...
  // Sidebar Preview defaults to ToonBoom
  ...
  // IPC HANDLER FOR MENU COMMANDS (4 Languages!)
  useEffect(() => {
    const handleMenuCommand = async (_event: any, command: string) => {
      if (['export-js', 'export-py', 'export-houdini', 'export-cs'].includes(command)) {
        let mode: 'js_toonboom' | 'py_maya' | 'py_houdini' | 'cs_csharp' = 'js_toonboom';
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }} onDrop={onDrop} onDragOver={onDragOver}>
        <ReactFlowProvider>
          <ReactFlow
            ...
            onNodeClick={(_e, n) => setSelectedNodeId(n.id)} onPaneClick={() => setSelectedNodeId(null)}
            onInit={setReactFlowInstance} deleteKeyCode={["Backspace", "Delete"]} snapToGrid fitView style={{ background: "#050505" }}
          >
