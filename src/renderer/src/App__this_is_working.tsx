...
// =============================================================================
//  CODE GENERATOR ENGINE (Updated for Smart Functions & Multi-Platform)
// =============================================================================
...
const generateCodeBlocks = (nodes: Node<FPNodeData>[], edges: Edge[], mode: 'js_toonboom' | 'py_maya' | 'py_houdini'): CodeBlock[] => {
  ...
  const generateNodeCode = (nodeId: string): string => {
    ...
    // Handle Function-based "Smart" translations
  ...
  // Only Toon Boom needs this specific footer block right now
  if (mode === 'js_toonboom') {
...
export default function App() {
  ...
  // This powers the real-time preview in your sidebar (defaulting to ToonBoom visually)
  ...
  // FIXED IPC HANDLER FOR MENU COMMANDS (Handles JS, Maya, and Houdini)
  useEffect(() => {
    const handleMenuCommand = async (_event: any, command: string) => {
      if (command === 'export-js' || command === 'export-py' || command === 'export-houdini') {
        let mode: 'js_toonboom' | 'py_maya' | 'py_houdini' = 'js_toonboom';
        let defaultName = 'tool.js';
        let ext = 'js';

        if (command === 'export-py') {
          mode = 'py_maya';
          defaultName = 'maya_script.py';
          ext = 'py';
        } else if (command === 'export-houdini') {
          mode = 'py_houdini';
          defaultName = 'houdini_script.py';
          ext = 'py';
        }
