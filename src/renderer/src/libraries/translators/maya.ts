...
if cmds.objExists("{camera_name}") and cmds.objExists("{mesh_name}"):
    ...
    # Clean the path for Maya MEL (Converts Windows backslashes to safe forward slashes)
    safe_path = r"{export_path}".replace("\\\\", "/").replace("\\\\", "/")
    ...
    # Force Maya to bake the camera animation
    mel.eval('FBXExportBakeComplexAnimation -v true')
    
    # Safely format and execute the MEL command
    mel_command = 'FBXExport -f "{}" -s'.format(safe_path)
    mel.eval(mel_command)
    
    print("FlowPins: FBX successfully exported to " + safe_path)
else:
...
# Clear selection so joints don't accidentally nest inside the wrong parent
cmds.select(clear=True) 

# Create the Joint at the exact coordinates
new_joint = cmds.joint(name=current_name, p=(x_pos, y_pos, 0))

# Auto-parent to the last created joint to build the bone chain
